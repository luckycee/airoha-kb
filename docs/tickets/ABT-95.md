...
之前解决过，在config tool 上调试PEQ， 使用LDAC无效的问题，此问题解决了（见 [ABT-91](ABT-91/)），现在客户发现使用App 新增自定义PEQ时，也无效；但是使用AAC和SBC有效，请继续指导，如何解决；
